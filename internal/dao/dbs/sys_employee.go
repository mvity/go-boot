...
type SysEmployee struct {
	Entity
	Operator
	Role          string `gorm:"column:B001;not null;index;size:128;comment:用户角色"`
	LoginKey      string `gorm:"column:B010;not null;index;size:32;comment:登录账号"`
	LoginPassword string `gorm:"column:B011;not null;size:128;comment:登录密码"`
...
// SysEmployeeFindByLoginKey 获取指定登录账号的用户
...
func SysEmployeeFindPager(db *gorm.DB, pager app.Pager, role string, loginKey string, name string) (*app.Paged, []*SysEmployee) {
	query := pager.GenQuery()
	query.AddSQL("SELECT C001 FROM SYS002 WHERE C003 = 0")
	if val := strings.TrimSpace(role); val != "" {
		query.AddSQLParam("AND B001 = ?", val)
	}
	if val := strings.TrimSpace(loginKey); val != "" {
		query.AddSQLParam("AND B010 = ?", val)
	}
	...
	paged, ids := findPager[SysEmployee](db, query)
